...
async def verify_page(token: str):
    html = HTML_TEMPLATE.replace("{{ token }}", token)
...
async def on_member_join(member: discord.Member):
    ...
    try:
        embed = discord.Embed(title=f"Welcome to {member.guild.name}!", 
                            description=f"Verify here:\n[**Click Here**]({verify_link})", 
                            color=discord.Color.blue())
        await member.send(embed=embed)
    except:
        print(f"Can't DM {member.name}")
...
async def main():
    ...
    # Replit automatically gives us the URL in an environment variable
    PUBLIC_URL = os.environ.get("REPL_SLUG") and f"https://{os.environ.get('REPL_SLUG')}.{os.environ.get('REPL_OWNER')}.repl.co"
    if not PUBLIC_URL:
        PUBLIC_URL = "http://localhost:8080" # Fallback for local testing
